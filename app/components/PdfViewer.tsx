    ...
    return (
      <div className="flex-1 flex items-center justify-center bg-surface-900/50">
        <div className="text-center">
          ...
          <p className="text-surface-400">{tr('editor.viewer.noPages', 'No pages to display')}</p>
        </div>
      </div>
    );
  }
      ...
      <div className="glass border-b border-surface-700/50 px-2 sm:px-4 py-2 sm:py-2.5">
        <div className="flex items-center justify-center gap-1 sm:gap-2">
          ...
          <button
            ...
            title={tr('editor.viewer.previousPage', 'Previous page')}
          >
          ...
          <div className="flex items-center gap-1 sm:gap-2 px-2 sm:px-3 py-1 sm:py-1.5 rounded-xl bg-surface-800/50">
            <span className="text-xs text-surface-400 font-medium hidden sm:inline">{tr('editor.viewer.page', 'Page')}</span>
            <select
          ...
          <button
            ...
            className="btn-icon-sm btn-ghost"
            title={tr('editor.viewer.nextPage', 'Next page')}
          >
            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          ...
          <div className="hidden lg:flex items-center gap-1 ml-4 text-xs text-surface-500">
            ...
            <kbd className="px-1.5 py-0.5 rounded bg-surface-700 text-surface-400 font-mono">→</kbd>
            <span className="ml-1">{tr('editor.viewer.navigateHint', 'to navigate')}</span>
          </div>
        </div>
      </div>
            ...
            {isRendering && (
              <div className="absolute inset-0 bg-surface-900/80 backdrop-blur-sm flex items-center justify-center z-10 rounded-lg">
                <div className="flex flex-col items-center gap-3">
                  ...
                  <span className="text-sm text-surface-300">{tr('editor.viewer.rendering', 'Rendering page...')}</span>
                </div>
              </div>
            )}
